...
public final class OwnerIds {
	...
	public static boolean isQualified(String value) {
		// "qualified" = a non-empty domain part precedes the separator, i.e. the
		// separator is present but NOT the first character (domainName must be non-empty).
		return value != null && !value.startsWith(SEPARATOR) && value.contains(SEPARATOR);
	}
}

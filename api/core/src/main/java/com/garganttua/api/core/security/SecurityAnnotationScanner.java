...
public final class SecurityAnnotationScanner {
    ...
    private void scanAuthenticators(IReflection reflection) throws ApiException {
        ...
        for (String pkg : this.packages) {
            for (IClass<?> authrClass : reflection.getClassesWithAnnotation(pkg, anno)) {
                wireAuthenticator(reflection, apiSec, authrClass.getAnnotation(anno), authrClass);
                registered++;
            }
        }
        if (registered > 0) {
    ...
    @SuppressWarnings({ "rawtypes", "unchecked" })
    private void wireAuthenticator(IReflection reflection, IApiSecurityBuilder apiSec,
            Authenticator a, IClass<?> authrClass) throws ApiException {
        ...
        wireAuthorizationDomain(a, authrClass, authrBuilder);
    ...
    @SuppressWarnings({ "rawtypes", "unchecked" })
    private void wireAuthorizationDomain(Authenticator a, IClass<?> authrClass,
            IAuthenticatorBuilder<Object> authrBuilder) throws ApiException {

 ...
 */
// AvoidFieldNameMatchingMethodName: intentional IClass descriptor mirror — each boolean
// property field (isInterface, isEnum, ...) backs its like-named accessor from the IClass contract.
...
public class AOTClass<T> implements IAOTClassDescriptor<T> {
    ...
    /** A shallow descriptor carries at most the synthesised no-arg constructor. */
    private static final int SHALLOW_CTOR_COUNT = 1;
    ...
    private volatile List<IClass<?>> resolvedInterfaces;
    ...
    public IClass<?>[] getInterfaces() {
        List<IClass<?>> cached = resolvedInterfaces;
        if (cached == null) {
            synchronized (this) {
                if (resolvedInterfaces == null) {
                    resolvedInterfaces = resolveInterfaceList();
                }
                cached = resolvedInterfaces;
            }
        }
        return cached.toArray(new IClass<?>[0]);
    }

    private List<IClass<?>> resolveInterfaceList() {
        List<IClass<?>> resolved = new ArrayList<>(interfaceNames.length);
        for (String interfaceName : interfaceNames) {
            try {
                resolved.add(IClass.forName(interfaceName));
            } catch (ClassNotFoundException e) {
                throw new IllegalStateException("Cannot resolve interface: " + interfaceName, e);
            }
        }
        return resolved;
    }
    ...
    public IConstructor<?>[] getDeclaredConstructors() {
        ...
        if (constructors.length > SHALLOW_CTOR_COUNT) return constructors.clone();
    ...
    @edu.umd.cs.findbugs.annotations.SuppressFBWarnings(
        ...
        justification = "Mirrors java.lang.Class.getRecordComponents(): null for non-record types.")
    // ReturnEmptyCollectionRatherThanNull: same java.lang.Class contract — null, not empty.
    ...
    @edu.umd.cs.findbugs.annotations.SuppressFBWarnings(
        value = "PZLA_PREFER_ZERO_LENGTH_ARRAYS",
        justification = "Mirrors java.lang.Class.getPermittedSubclasses(): null for non-sealed types.")
    // ReturnEmptyCollectionRatherThanNull: same java.lang.Class contract — null, not empty.
    @SuppressWarnings("PMD.ReturnEmptyCollectionRatherThanNull")
    ...
    @Override
    // UseProperClassLoader: the IClass.getClassLoader() contract intentionally returns the
    // *defining* loader of the described type, not the thread context loader.
    ...
    @edu.umd.cs.findbugs.annotations.SuppressFBWarnings(
        ...
        justification = "Descriptor identity is the binary name: an AOTClass is intentionally "
            + "equal to the IClass/Class it describes, so cross-type comparison is by design.")

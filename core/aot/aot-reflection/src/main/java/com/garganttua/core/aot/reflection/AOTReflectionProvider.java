...
public class AOTReflectionProvider implements IReflectionProvider {
    ...
    // PreserveStackTrace: the unresolved rethrow chains 'notReachable' (the real classloader miss)
    // as cause; the outer 'intrinsicMiss' is the expected primitive/array probe failure, not the
    // root cause, so it is intentionally not chained.

...
final class WorkflowVariableCollector {
    ...
    /** Maximum rendered length of a variable value in the error dump before truncation. */
    private static final int MAX_DUMP_VALUE_LENGTH = 120;
    ...
    private static final int MAX_EXCEPTION_CHAIN_DEPTH = 10;
    ...
    static void logErrorDump(String workflowName, IScript script, String scriptSource,
        ...
        appendExceptionChain(dump, exception);
        appendScriptErrorContext(dump, script);
        appendFailurePointVariables(dump, script, stagesToCollect);
    ...
    private static void appendExceptionChain(StringBuilder dump, Throwable exception) {
        if (exception == null) {
            return;
        }
        dump.append("║\n║  Exception chain:\n");
        ...
        int depth = 0;
        while (t != null && depth < MAX_EXCEPTION_CHAIN_DEPTH) {
    ...
    private static void appendScriptErrorContext(StringBuilder dump, IScript script) {
    ...
    private static void appendFailurePointVariables(StringBuilder dump, IScript script,
        ...
        for (var entry : vars.entrySet()) {
            ...
            if (val.length() > MAX_DUMP_VALUE_LENGTH) {
                val = val.substring(0, MAX_DUMP_VALUE_LENGTH) + "...";
            }

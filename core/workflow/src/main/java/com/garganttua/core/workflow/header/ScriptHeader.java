...
public record ScriptHeader(
    ...
    public ScriptHeader {
        ...
        // List.copyOf / Map.copyOf yield genuinely immutable, defensively-copied
        // collections — safe to expose via the record accessors (no EI_EXPOSE_REP).
        inputs = inputs != null ? List.copyOf(inputs) : List.of();
        outputs = outputs != null ? List.copyOf(outputs) : List.of();
        returnCodes = returnCodes != null ? Map.copyOf(returnCodes) : Map.of();
        catchExpression = trimOrNull(catchExpression);
        catchDownstreamExpression = trimOrNull(catchDownstreamExpression);
    }

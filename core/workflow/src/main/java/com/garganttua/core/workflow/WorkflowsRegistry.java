...
public class WorkflowsRegistry implements IBootstrapSummaryContributor, Map<String, IWorkflow> {
    ...
    private static final String IMMUTABLE_MESSAGE = "WorkflowsRegistry is immutable";

    /** Maximum rendered length of the comma-joined workflow-name summary before truncation. */
    private static final int MAX_NAMES_SUMMARY_LENGTH = 50;
    ...
    public Map<String, String> getSummaryItems() {
        ...
        if (!this.workflows.isEmpty()) {
            ...
            if (names.length() > MAX_NAMES_SUMMARY_LENGTH) {
                names = names.substring(0, MAX_NAMES_SUMMARY_LENGTH - 3) + "...";
            }

...
public class ScriptContext implements IScript, IObservable {
    ...
    private static final String OBS_COMPILE = "scriptcontext:compile";
    private static final String OBS_EXECUTE = "scriptcontext:execute";
    ...
    @Override
    // null clears the stale compiled runtime — an intentional state reset, not a smell
    ...
    public void load(InputStream inputStream) throws ScriptException {
        ...
        try {
            this.load(new String(inputStream.readAllBytes(), java.nio.charset.StandardCharsets.UTF_8));
        } catch (IOException e) {
    ...
    public void compile() throws ScriptException {
        ...
        try (ObservabilityEmitter.Scope scope = ObservabilityEmitter.open(this.observers, UUID.randomUUID())) {
            scope.fireStart(OBS_COMPILE);
            try {
                doCompile();
                scope.fireEnd(OBS_COMPILE);
            } catch (ScriptException e) {
                scope.fireError(OBS_COMPILE, e);
                throw e;
    ...
    private void doCompile() throws ScriptException {
        this.runtime = new ScriptCompiler(this.expressionContext, this.runtimesBuilderFactory,
                this.initialVariables).compile(this.scriptSource);
    }
    ...
    @SuppressFBWarnings(value = "THROWS_METHOD_THROWS_RUNTIMEEXCEPTION",
            justification = "Re-throws the original RuntimeException after firing an error observability event.")
    // null/false here reset per-run state before execution — intentional, not a smell
    @SuppressWarnings("PMD.NullAssignment")
    public int execute(Object... args) throws ScriptException {
        ...
        try (ObservabilityEmitter.Scope scope = ObservabilityEmitter.open(this.observers, UUID.randomUUID())) {
            scope.fireStart(OBS_EXECUTE);
            int code;
            try {
                code = doExecute(args);
            } catch (ScriptException e) {
                scope.fireError(OBS_EXECUTE, e);
                throw e;
            } catch (RuntimeException e) {
                scope.fireError(OBS_EXECUTE, e);
                throw e;
            }
            scope.fireEnd(OBS_EXECUTE, code);
            return code;
        }
    }

    // result fields are assigned in the try then reset on the catch path; null/empty are
    // intentional "no result" markers, and PMD's overwrite warning is a try/catch false positive
    @SuppressWarnings({"PMD.NullAssignment", "PMD.UnusedAssignment"})
     ...
     */
    @SuppressFBWarnings(value = "EI_EXPOSE_REP",
            justification = "Class-loader manager is a shared service exposed by reference by design, not copied.")

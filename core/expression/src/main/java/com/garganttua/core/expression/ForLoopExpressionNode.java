...
public class ForLoopExpressionNode implements IExpressionNode<Object, ISupplier<Object>> {
    ...
    public ISupplier<Object> evaluate() throws ExpressionException {
        return new ForLoopSupplier();
    }

    /** Runs the loop on demand, returning the value of the last body evaluation. */
    private final class ForLoopSupplier implements ISupplier<Object> {
        @Override
    ...
    private Object runLoop() throws SupplyException {
        Object lastResult = null;
        int iterations = 0;
        while (iterations < MAX_ITERATIONS) {
            Object condResult = conditionNode.evaluate().supply().orElse(null);
            if (!(condResult instanceof Boolean b && b)) {
                break;
            }
            lastResult = bodyNode.evaluate().supply().orElse(null);
            Object updateResult = updateNode.evaluate().supply().orElse(null);

...
final class ExpressionVisitor extends com.garganttua.core.expression.antlr4.ExpressionBaseVisitor<IExpressionNode<?, ? extends ISupplier<?>>> {
    ...
    /** Reserved function name that introduces a {@code for(...)} loop expression. */
    private static final String FOR_KEYWORD = "for";
    /** Reserved {@code .class} type-literal selector. */
    ...
    public IExpressionNode<?, ? extends ISupplier<?>> visitVariableReference(
        ...
        ExpressionNode node = new ExpressionNode(nodeName,
                (IEvaluateNode) (params) -> new VariableSupplier(nodeName, varName, eagerEval, resolvedIClass),
                (IClass) resolvedIClass);
        return node;
    }

    /** Resolves a variable (or argument) reference against the active resolver at supply time. */
    private static final class VariableSupplier implements ISupplier<Object> {
        private final String nodeName;
        private final String varName;
        private final boolean eagerEval;
        private final IClass<?> resolvedIClass;

        VariableSupplier(String nodeName, String varName, boolean eagerEval, IClass<?> resolvedIClass) {
            this.nodeName = nodeName;
            this.varName = varName;
            this.eagerEval = eagerEval;
            this.resolvedIClass = resolvedIClass;
        }
        ...
        public java.util.Optional<Object> supply() throws com.garganttua.core.supply.SupplyException {
            ...
            if (eagerEval && resolved.isPresent()) {
                return eagerEvaluate(resolved.get(), resolved);
            }
            return resolved;
        }

        private java.util.Optional<Object> eagerEvaluate(Object value, java.util.Optional<Object> fallback)
            ...
            if (value instanceof IExpression<?, ?> expr) {
                ...
                return expr.evaluate().supply().map(r -> (Object) r);
            }
            return fallback;
        }
    ...
    public IExpressionNode<?, ? extends ISupplier<?>> visitFunctionCall(ExpressionParser.FunctionCallContext ctx) {
        ...
        return visitClassicFunctionCall(ctx);
    }

    private IExpressionNode<?, ? extends ISupplier<?>> visitClassicFunctionCall(
        ...
        if (FOR_KEYWORD.equals(functionName)) {
            return visitForLoop(ctx);
        }

        List<Object> arguments = collectArguments(ctx, functionName);
        ...
        if (factory == null) {
            if (dynamicFunctionsEnabled && !keys.hasRegisteredFunction(functionName)) {
                return createDynamicFunctionNode(functionName, arguments);
            }
    ...
    private List<Object> collectArguments(ExpressionParser.FunctionCallContext ctx, String functionName) {
    ...
    private IExpressionNode<?, ? extends ISupplier<?>> createDynamicFunctionNode(String functionName,
            List<Object> arguments) {
        // Fallback: create a DynamicFunctionNode that resolves the function from runtime
        // variables (supports user-defined script functions). Only reached when no registered
        // factory exists with this name (avoids masking type mismatches).
    ...
    public IExpressionNode<?, ? extends ISupplier<?>> visitType(ExpressionParser.TypeContext ctx) {
        ...
        if (ctx.arrayDims() == null && ctx.simpleType() != null) {
            ...
            if (simpleType.classType() != null) {
                ...
                return createNode(CLASS_KEYWORD, className);
            }
        }
    ...
    private String getFullClassName(ExpressionParser.ClassTypeContext ctx) {
        if (ctx.className() != null) {
            ...
            int end = (size > 1 && CLASS_KEYWORD.equals(ctx.className().IDENTIFIER(size - 1).getText()))
                    ? size - 1
                    : size;
            for (int i = 0; i < end; i++) {
                if (i > 0)

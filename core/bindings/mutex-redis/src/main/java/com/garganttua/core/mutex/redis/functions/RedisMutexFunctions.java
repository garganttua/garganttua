...
public final class RedisMutexFunctions {
    ...
    public static Object syncRedis(@Nullable String mutexName, @Nullable ISupplier<?> expression) {
        ...
        try {
            ...
            throw wrap("syncRedis: mutex operation failed - " + e.getMessage(), e);
        } catch (Exception e) {
            log.error("syncRedis: expression evaluation failed", e);
            throw wrap("syncRedis: expression evaluation failed - " + e.getMessage(), e);
        }
    }

    /**
     * Builds an {@link ExpressionException} with the given message while preserving the
     * stack trace of the original cause.
     *
     * @param message the diagnostic message
     * @param cause   the underlying exception to preserve
     * @return the cause-preserving exception, ready to be thrown
     */

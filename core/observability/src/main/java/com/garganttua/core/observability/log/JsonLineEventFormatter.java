...
public final class JsonLineEventFormatter implements IEventFormatter {
    ...
    /** JSON key for the discriminating event type field. */
    ...
    private static void appendTypeFields(StringBuilder sb, ObservableEvent event) {
        switch (event) {
            ...
            case ErrorEvent x -> appendErrorFields(sb, x);
            case LogEvent l -> {
    ...
    private static void appendErrorFields(StringBuilder sb, ErrorEvent x) {
    ...
    // justification: 0x20 is the JSON spec's control-char escaping boundary, not a config literal

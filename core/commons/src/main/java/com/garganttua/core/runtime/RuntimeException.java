...
public class RuntimeException extends CoreException {
    ...
    private static final String EXITING_CTOR = "Exiting RuntimeException constructor";

    /**
     * Optional reference to the runtime context where this exception occurred.
     */
    // field intentionally mirrors the context() accessor
    @SuppressWarnings("PMD.AvoidFieldNameMatchingMethodName")
    private transient Optional<IRuntimeContext<?, ?>> context = Optional.empty();

    /**
     * @return the runtime context attached to this exception, or empty when none
     *         was provided (or after serialization, since the context is transient)
     */
    public Optional<IRuntimeContext<?, ?>> context() {
        return this.context == null ? Optional.empty() : this.context;
    }

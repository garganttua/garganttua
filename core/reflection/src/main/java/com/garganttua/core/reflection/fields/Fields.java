...
public class Fields {
	...
	public static class BlackList {
		...
		private static List<IClass<?>> blackListedClasses = new CopyOnWriteArrayList<>();
		...
		public static void addClassToBlackList(IClass<?> clazz) {
			...
			BlackList.blackListedClasses.add(clazz);
		}
		...
		public static boolean isBlackListed(IClass<?> clazz) {
			Optional<IClass<?>> found = BlackList.blackListedClasses.stream().filter(cl -> cl.equals(clazz)).findFirst();
	...
	// ReplaceVectorWithList: Vector is the documented, test-pinned fallback for a bare Collection interface

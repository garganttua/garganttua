 ...
 */
// PMD.NullAssignment: fluent setters intentionally reset the mutually-exclusive pending-field/value state to null
...
public abstract class AbstractFieldBinderBuilder<FieldType, OwnerType, Builder extends IFieldBinderBuilder<FieldType, OwnerType, Builder, Link>, Link>
    ...
    private static final Logger log = Logger.getLogger(AbstractFieldBinderBuilder.class);

    private IClass<OwnerType> ownerType;
    protected ObjectAddress address;
    protected ISupplierBuilder<?, ?> valueSupplierBuilder;
    // protected field type retained as part of the subclass-facing builder API (api/events extend this)
    @SuppressFBWarnings(value = "URF_UNREAD_PUBLIC_OR_PROTECTED_FIELD",
            justification = "Protected builder API field consumed by api/events subclasses, not in this module.")
    ...
    // AvoidFieldNameMatchingMethodName: backs the fluent allowNull(boolean) DSL setter by design
    ...
    @SuppressFBWarnings(value = "EQ_CHECK_FOR_OPERAND_NOT_COMPATIBLE_WITH_THIS",
            justification = "Intentional address-based identity for dependency dedup (see class javadoc); compared against the target field address, not the builder type.")
    ...
    @SuppressFBWarnings(value = "EI_EXPOSE_REP2",
            justification = "Builder intentionally retains the caller-supplied IField until doBuild() resolves it.")
    public IFieldBinderBuilder<FieldType, OwnerType, Builder, Link> field(

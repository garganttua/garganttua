 ...
 *
 * <p>
 * <b>Size note:</b> this class slightly exceeds the 500-line god-class gate
 * (~512 lines). Its get/set logic is a single cohesive recursive walk over the
 * resolved address, tightly coupled to instance state (resolved field, address,
 * recursion indices) and the array/map/collection traversal helpers — extracting
 * a collaborator would mean threading that state through, for marginal benefit.
 * It is therefore an approved exception (registered in
 * {@code config/checkstyle/suppressions.xml}); a proper get/set split remains a
 * future option once the recursion is covered by more targeted tests.
 * </p>
 *
 * @param <T> the type of the addressed field value
 */
// PMD suppressions for this recursive accessor:
//  - UnusedLocalVariable: try-with-resources access guards are used for their close(), not their value
//  - AvoidLiteralsInIfCondition: idiomatic single-element field-path size checks (== 1)
//  - ForLoopCanBeForeach: index loops iterate a parallel source list alongside a separate iterator
@SuppressWarnings({"PMD.UnusedLocalVariable", "PMD.AvoidLiteralsInIfCondition", "PMD.ForLoopCanBeForeach"})
public class FieldAccessor<T> implements ISupplier<IFieldValue<T>> {
	...
	public FieldAccessor(ResolvedField resolvedField, boolean force) throws ReflectionException {
		...
		this.force = force;
		log.debug("FieldAccessor initialized for ownerType={}, address={}, force={}", ownerType.getName(), address, force);
	}
	...
	public IFieldValue<T> getValue(Object object) throws ReflectionException {
		...
		if (fieldPath.size() == 1) {
			...
			log.debug("Single field access: fieldName={}", fieldName);
			checkFieldNameMatch(field, fieldName);
			T value = (T) getFieldValue(object, field);
			log.debug("Successfully retrieved value for address={}", this.address);
			return SingleFieldValue.of(value, fieldType);
		} else {
			log.debug("Recursive field access with {} fields", fieldPath.size());
	...
	private IFieldValue<T> getValueRecursively(Object object, int addressIndex, int fieldIndex) throws ReflectionException {
		...
		log.trace("getValueRecursively: addressIndex={}, fieldIndex={}, lastIteration={}", addressIndex, fieldIndex, isLastIteration);
		...
		if (temp == null) {
			log.debug("Field {} value is null, returning null", fieldName);
		...
		if (Fields.isArrayOrMapOrCollectionField(field)) {
			...
			if (isLastIteration) {
				log.debug("Last iteration, getting final field value");
	...
	private void doGetIfIsArray(List<SingleFieldValue<T>> collected, IField field, IField nextField, Object list,
		...
		if (field.getType().isArray()) {
			log.debug("Processing array field for get: {}", field.getName());
	...
	private void doGetIfIsMap(List<SingleFieldValue<T>> collected, String nextFieldName, IField field, IField nextField, Object list,
		...
		if (Map.class.isAssignableFrom(rawType)) {
			log.debug("Processing map field for get: {}, nextFieldName={}", fieldName, nextFieldName);
			Map<Object, Object> map = (Map<Object, Object>) list;
			if (ObjectAddress.MAP_KEY_INDICATOR.equals(nextFieldName)) {
				for (Object obj : map.keySet()) {
					processGetArrayElement(collected, nextField, isLastIteration, addressIndex, fieldIndex, obj, 1, 2);
				}
			} else if (ObjectAddress.MAP_VALUE_INDICATOR.equals(nextFieldName)) {
	...
	private void doGetIfIsCollection(List<SingleFieldValue<T>> collected, IField field, IField nextField, Object list,
		...
		if (Collection.class.isAssignableFrom(rawType)) {
			log.debug("Processing collection field for get: {}", field.getName());
	...
	public IFieldValue<T> setValue(Object object, IFieldValue<T> value) throws ReflectionException {
		...
		if (this.fieldPath.size() == 1) {
			...
			String fieldName = this.address.getElement(0);
			log.debug("Direct field assignment: fieldName={}", fieldName);
			checkFieldNameMatch(field, fieldName);
			setFieldValue(object, field, value.first());
		} else {
			log.debug("Recursive field assignment with {} fields", this.fieldPath.size());
			this.setValueRecursively(object, value.first(), 0, 0);
		}
		log.debug("Successfully set value for address={}", this.address);
		return value;
	}
	...
	public IFieldValue<T> setValue(IFieldValue<T> fieldValue) throws ReflectionException {
		...
		log.debug("Created new instance of {}", this.ownerType.getName());
	...
	private void setValueRecursively(Object object, Object value, int fieldIndex, int fieldNameIndex)
			throws ReflectionException {
		log.trace("setValueRecursively: fieldIndex={}, fieldNameIndex={}", fieldIndex, fieldNameIndex);
		boolean isLastIteration = (fieldIndex + 2 == this.fieldPath.size());
		IField field = (IField) this.fieldPath.get(fieldIndex);
		IField nextField = (IField) this.fieldPath.get(fieldIndex + 1);
		...
		Object temp = getFieldValue(object, field);
		if (temp == null) {
			log.debug("Field {} is null, instantiating new object", fieldName);
		...
		if (Fields.isArrayOrMapOrCollectionField(field)) {
			...
			if (isLastIteration) {
				log.debug("Last iteration, setting final field value");
	...
	// object retained for signature uniformity with the doSetIf* dispatch family (see set dispatch)
	...
	private void doSetIfIsMap(Object object, Object value, int fieldIndex, int fieldNameIndex,
		...
		Class<?> rawType = (Class<?>) field.getType().getType();
		if (Map.class.isAssignableFrom(rawType)) {
			log.debug("Processing map field for set: {}", field.getName());
	...
	private Iterator<?> mapEntryIterator(Map<Object, Object> target, String nextFieldName) throws ReflectionException {
		...
		if (ObjectAddress.MAP_VALUE_INDICATOR.equals(nextFieldName)) {
	...
	private void doSetIfIsArray(Object object, Object value, int fieldIndex, int fieldNameIndex,
			...
			throws ReflectionException {
		if (field.getType().isArray()) {
			log.debug("Processing array field for set: {}", field.getName());
	...
	private void doSetIfIsCollection(Object object, Object value, int fieldIndex, int fieldNameIndex,
		...
		Class<?> rawType = (Class<?>) field.getType().getType();
		if (Collection.class.isAssignableFrom(rawType)) {
			log.debug("Processing collection field for set: {}", field.getName());

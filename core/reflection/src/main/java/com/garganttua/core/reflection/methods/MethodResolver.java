 ...
 */
// PMD suppressions for this resolver:
//  - AvoidLiteralsInIfCondition: idiomatic match-count comparisons against 1 (unique / ambiguous overload)
//  - UseVarargs: signature matchers distinguish null (no constraint) from empty (zero params), so the
//    explicit IClass<?>[] parameter must not collapse into varargs
@SuppressWarnings({"PMD.AvoidLiteralsInIfCondition", "PMD.UseVarargs"})
public class MethodResolver {
    private static final Logger log = Logger.getLogger(MethodResolver.class);

    private static final String IN_OWNERTYPE = " in ownertype ";
        ...
        public static ResolvedMethod methodByName(IClass<?> ownerType, IReflectionProvider provider,
                ...
                if (found.size() > 1) {
                        ...
                        throw new ReflectionException("Multiple overloads of method " + methodName + IN_OWNERTYPE
                ...
                if (found.isEmpty()) {
                        log.error(
                                        ...
                                        methodName, ownerType.getName());
                        throw new ReflectionException("No overload of method " + methodName + IN_OWNERTYPE
        ...
        public static ResolvedMethod methodByName(IClass<?> ownerType, IReflectionProvider provider,
                ...
                if (found.isEmpty()) {
                        ...
                        throw new ReflectionException(
                                        "No overload of method " + methodName + IN_OWNERTYPE + ownerType.getName());
                }
                return found.get(0);
        }
        ...
        public static ResolvedMethod methodByMethod(IClass<?> ownerType, IReflectionProvider provider,
                ...
                if (found.size() > 1) {
                        throw new ReflectionException("Multiple overloads of method " + method.getName()
                                        + IN_OWNERTYPE + ownerType.getName() + " match the exact signature");
                }
                if (found.isEmpty()) {
                        throw new ReflectionException("No overload of method " + method.getName()
                                        + IN_OWNERTYPE + ownerType.getName() + " matches the exact signature");
                }
                return found.get(0);
        }

...
public record ResolvedMethod(ObjectAddress address, List<Object> methodPath) implements IMethod {
    ...
    /**
     * Canonical constructor taking a defensive, immutable copy of {@code methodPath}
     * so the record neither stores nor exposes a caller-mutable list.
     */
    public ResolvedMethod {
        methodPath = List.copyOf(methodPath);
    }
     ...
     */
    // UseVarargs: null (no constraint) and empty (zero params) are distinct here, so the explicit array stays

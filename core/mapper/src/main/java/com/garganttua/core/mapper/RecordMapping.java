...
final class RecordMapping {
    ...
    private static final Logger log = Logger.getLogger(RecordMapping.class);
    ...
    private static void readSourceValues(IReflection reflection, IRecordComponent[] components, Object source,
        ...
        for (IRecordComponent component : components) {
            ...
            try {
                ...
                // Field not found in source: keep the default value for this component.
                log.debug("No source field for record component {}: {}", name, e.getMessage());
            }
        }
    }

    /** Build the record instance via its canonical constructor using the collected component values. */
    // unchecked: generic record cast. PMD.AvoidAccessibilityAlteration: setAccessible is required
    // to invoke a record's canonical constructor reflectively.
    @SuppressWarnings({ "unchecked", "PMD.AvoidAccessibilityAlteration" })
    // throws Exception: aggregates the several checked reflection exceptions of the canonical-ctor path.
    @SuppressFBWarnings(value = "THROWS_METHOD_THROWS_CLAUSE_BASIC_EXCEPTION",
            justification = "Reflective record instantiation aggregates several checked exceptions; caught generically in map().")

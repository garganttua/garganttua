...
public final class GarganttuaVersion {
    ...
    private static synchronized void loadProperties() {
        ...
        try (InputStream is = openProperties()) {
            if (is != null) {
                PROPERTIES.load(is);
    ...
    /**
     * Opens the version properties resource, preferring the thread context
     * class loader (so the resource resolves in container / modular setups)
     * and falling back to this class's own loader when the context loader
     * cannot see it.
     */
    private static InputStream openProperties() {
        ClassLoader contextLoader = Thread.currentThread().getContextClassLoader();
        if (contextLoader != null) {
            InputStream is = contextLoader.getResourceAsStream(PROPERTIES_FILE);
            if (is != null) {
                return is;
            }
        }
        return GarganttuaVersion.class.getResourceAsStream("/" + PROPERTIES_FILE);
    }

...
final class BootstrapConsoleReporter {
    ...
    private static PrintStream createUtf8ConsoleStream() {
        try {
            ...
            // Defensive: fall through to wrap System.out in a UTF-8 stream.
        }
        try {
    ...
    static void printSummary(BannerMode bannerMode, String applicationName, String applicationVersion,
        ...
        addTimingBreakdown(summary, lastBuildTimings);
        emitSummary(bannerMode, summary);
    }

    /**
     * Append the per-stage timing breakdown to the summary: phase totals (keys
     * without {@code ':'}) followed by per-builder entries, slowest first.
     */
    private static void addTimingBreakdown(BootstrapSummary summary, StageTimings lastBuildTimings) {
    ...
    /** Render the summary to the console (CONSOLE) or to the log (LOG); no-op for OFF. */
    private static void emitSummary(BannerMode bannerMode, BootstrapSummary summary) {

...
public class ScriptConsole {
     ...
     */
    // EI_EXPOSE_REP2: caller-provided I/O streams (and reader) are written to/read from live; copying them is nonsensical.
    @SuppressFBWarnings(value = "EI_EXPOSE_REP2",
            justification = "Caller-owned I/O streams are used live; defensive copy is impossible/meaningless.")
    ...
    private void executeStatement(String statement) {
        try {
            // No custom class-loader manager is wired in the REPL.
            ScriptContext script = new ScriptContext(expressionContext,
                    () -> RuntimesBuilder.builder().provide(injectionContextBuilder),
                    (com.garganttua.core.classloader.IClassLoaderManager) null);
    ...
    private void collectVariables(IScript script, String statement) {
        ...
        for (String rawLine : lines) {
            ...
            if (eqIndex > 0 && arrowIndex < 0) {
                ...
                if (line.charAt(eqIndex - 1) != '-' && line.charAt(eqIndex - 1) != '=' &&

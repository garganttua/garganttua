...
public class ConsoleFunctions {
    ...
    public static int load(@Nullable String filename) {
        ...
        try {
            // Read the file to validate it is accessible before reporting the unsupported mode.
            ...
            // PreserveStackTrace: ExpressionException has no (String, Throwable) ctor; the cause
            // is surfaced via getMessage() and chained with initCause to retain the stack trace.
            ExpressionException ee = new ExpressionException("load: error reading file: " + e.getMessage());
            ee.initCause(e);
            throw ee;
        }
    }
    ...
    private static String buildSyntaxText() {
        StringBuilder sb = new StringBuilder();
        ...
        sb.append("\n");
        appendSyntaxHandlingSection(sb);
        return sb.toString();
    }

    private static void appendSyntaxHandlingSection(StringBuilder sb) {
    ...
    private static void displayPaginated(String content, ConsoleContext ctx) {
        ...
        while (currentLine < totalLines) {
            ...
            if (currentLine < totalLines) {
                ...
                out.print(buildPaginationPrompt(percent, linesRemaining, colorsEnabled));
                out.flush();

                if (shouldStopPaging(out, lineReader)) {
                    break;
                }
            }
        }
    ...
    private static String buildPaginationPrompt(int percent, int linesRemaining, boolean colorsEnabled) {
        if (colorsEnabled) {
            return BRIGHT_BLACK + "── " + RESET +
        ...
        return "── " + percent + "% ── " + linesRemaining + " lines remaining ── [Enter: next, q: quit] ──";
    }

    /** Reads the pagination keypress; returns {@code true} when paging should stop (quit/Ctrl-C/Ctrl-D). */
    private static boolean shouldStopPaging(PrintStream out, LineReader lineReader) {
        try {
            String input = lineReader.readLine("");
            if (input != null) {
                input = input.trim().toLowerCase(java.util.Locale.ROOT);
                return "q".equals(input) || "quit".equals(input) || "exit".equals(input);
            }
            return false;
        } catch (UserInterruptException | EndOfFileException e) {
            // Ctrl+C / Ctrl+D pressed - exit pagination
            out.println();
            return true;
        }
    }

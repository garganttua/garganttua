...
public class BuilderDependency<Builder extends IObservableBuilder<Builder, Built>, Built>
    ...
    private static final String LOG_READY_EXEC = "Dependency is ready, executing consumer";

    private final IClass<Builder> dependencyClass;
    // Field names mirror the fluent accessors spec()/builder() by design.
     ...
     * {@code rebuild()}.
     */
    // EnumSet is the intentional implementation type: compact bitset semantics for the firing-event set.
    ...
    public BuilderDependency(IClass<? extends IObservableBuilder<?, ?>> dependencyClass, DependencySpec spec) {
        ...
        // At construction both builder and builtObject are null, so the dependency
        // is never ready and always empty; using literals avoids calling overridable
        // methods (isReady/isEmpty) from the constructor.
        log.debug("BuilderDependency created for: {}, phase: {}, isReady: {}, isEmpty: {}",
            this.dependencyClass.getName(), spec.phase(), false, true);
    }

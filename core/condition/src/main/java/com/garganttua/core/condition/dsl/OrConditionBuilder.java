...
/**
 * Builds an {@link OrCondition} that is satisfied when at least one supplied
 * sub-condition evaluates to {@code true}.
 */
@Reflected
public final class OrConditionBuilder implements IConditionBuilder {
    private static final Logger log = Logger.getLogger(OrConditionBuilder.class);

    private final IConditionBuilder[] conditions;

    /**
     * Creates a builder over the operand conditions to be OR-combined.
     *
    ...
    /**
     * Builds the OR condition, or {@code null} when this builder is contextual
     * (deferred resolution).
     *
     * @return the composed {@link OrCondition}, or {@code null} if contextual

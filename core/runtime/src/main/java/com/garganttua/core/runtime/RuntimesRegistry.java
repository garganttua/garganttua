...
public class RuntimesRegistry implements IBootstrapSummaryContributor, Map<String, IRuntime<?, ?>> {
    ...
    /** Message used when an unsupported mutating {@link Map} operation is attempted. */
    private static final String IMMUTABLE_MESSAGE = "RuntimesRegistry is immutable";
    /** Maximum length of the runtime-names summary line before it is truncated. */
    private static final int NAMES_SUMMARY_MAX_LENGTH = 50;
    /** Number of characters retained before the truncation ellipsis. */
    private static final int NAMES_SUMMARY_TRUNCATE_AT = 47;
    ...
    public Map<String, String> getSummaryItems() {
        ...
        if (!runtimes.isEmpty()) {
            ...
            if (names.length() > NAMES_SUMMARY_MAX_LENGTH) {
                names = names.substring(0, NAMES_SUMMARY_TRUNCATE_AT) + "...";
            }

...
public class CatchAwareExpression<R> implements IExpression<R, ISupplier<R>> {
     ...
     */
    // AvoidFieldNameMatchingMethodName: handler()/code()/variableName() are intentional accessors over same-named fields.
    ...
    private class CatchAwareSupplier implements ISupplier<R> {
        ...
        @Override
        // PreserveStackTrace false positive: cre is rethrown as-is and handlerEx is passed as the cause.

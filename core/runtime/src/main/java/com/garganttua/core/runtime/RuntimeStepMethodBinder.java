...
public class RuntimeStepMethodBinder<ExecutionReturned, InputType, OutputType>
    ...
    public void execute(IRuntimeContext<InputType, OutputType> context,
        ...
        try {
            ...
            this.handleCatchResult(context, variable, cre);
            return; // Don't call next.execute() — chain stops
        } catch (Exception e) {
            this.handleExecutionException(context, variable, e);
        }
    ...
    private void handleCatchResult(IRuntimeContext<InputType, OutputType> context, Optional<String> variable,
            CatchAwareExpression.CatchResultException cre) {
        // Catch handler matched and executed — extract result, stop chain
        log.debug("{}Catch handler matched, stopping chain", logLineHeader());
        ExecutionReturned returned = (ExecutionReturned) cre.getResult();
    ...
    private void handleExecutionException(IRuntimeContext<InputType, OutputType> context, Optional<String> variable,
            Exception e) {
        ...
        if (!forceAbort) {
            ...
            processExecutionReturn(context, variable, null);
        }
    }
    ...
    private ExecutionReturned evaluatePipes(IRuntimeContext<InputType, OutputType> context,
        ...
        for (IRuntimeStepPipe pipe : pipes) {
            if (pipeMatches(pipe)) {
                ...
                return executePipeHandler(context, pipe, currentResult);
            }
        }
        return currentResult;
    }

    private boolean pipeMatches(IRuntimeStepPipe pipe) {
        ...
        try {
            ...
            return condResult instanceof Boolean b && b;
        } catch (Exception e) {
    ...
    @SuppressWarnings("unchecked")
    private ExecutionReturned executePipeHandler(IRuntimeContext<InputType, OutputType> context,
            IRuntimeStepPipe pipe, ExecutionReturned currentResult) {
        try {

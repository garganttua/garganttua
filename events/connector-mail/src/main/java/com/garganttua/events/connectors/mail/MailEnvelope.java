 ...
 * recipient / subject / body instead of being pinned to the connector configuration.
 *
 * <p>The expected JSON shape is:</p>
 * <pre>{ "to": "...", "from": "...", "subject": "...", "contentType": "text/html", "body": "..." }</pre>
 *
 * <p>Any field may be omitted — {@link MailSender} falls back to the connector config for absent
 * fields. A payload that is not a JSON object is treated as a plain-text body (backwards
 * compatibility), in which case {@link #tryParse} returns {@link Optional#empty()}.</p>
 *
